...
export class OCRService {
  async recognizeCard(imageBase64: string): Promise<OCRResult> {
    ...
    try {
      ...
      const passes = [namePass, nameAltPass, fullPass, numberPass].filter(Boolean) as OCRPass[];
...
async function buildRegions(input: Buffer): Promise<{ full: Buffer; name: Buffer; nameAlt: Buffer; number: Buffer }> {
...
async function recognizeRegion(worker: Awaited<ReturnType<typeof createWorker>>, input: Buffer, psm: PSM): Promise<OCRPass> {
...
function averageConfidence(passes: OCRPass[]): number {
  if (!passes.length) {
    return 0;
  }
